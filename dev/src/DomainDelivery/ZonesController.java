...
public class ZonesController extends DeliveriesController {
    ...
    public String addShippingZone(int id, String name) {
        DomainDelivery.Shipping_Zone existingZone = findZoneByName(name);

        if (existingZone != null) {
            return "Zone with name " + name + " already exists.\nCurrent zone rank: " + existingZone.getNum();
        } else {
            DomainDelivery.Shipping_Zone newZone = new DomainDelivery.Shipping_Zone(id, name);
    ...
    private static DomainDelivery.Shipping_Zone findZoneByName(String name) {
    ...
    public String deleteZone(String name) {
        DomainDelivery.Shipping_Zone zone = findZoneByName(name);
        if (zone == null) {
