...
//            displayError("Possible reasons:");
//            displayError("1. Shifts already exist for this week and branch");
//            displayError("2. No shift managers are available for this branch");
//        } else {
...
public class ShiftSchedulingScreen extends BaseScreen {
    ...
    private void createShiftsForWeekByBranch() {
        ...
        if (createdShifts.isEmpty()) {
            ...
            displayError("Possible reasons:");
            displayError("1. Shifts already exist for this week and branch");
            displayError("2. Database connection error");
        } else {
            displayMessage("SUCCESS: " + createdShifts.size() + " shifts were created successfully for branch " +
                    selectedBranch.getAddress() + " for week starting " + startDate.format(dateFormatter));

            // הצג את המשמרות שנוצרו עם מנהל המשמרת
            displayMessage("\nCreated shifts:");
            for (ShiftDTO shift : createdShifts) {
                String managerName = shift.hasShiftManager() ? shift.getShiftManagerName() : "No Manager";
                displayMessage("- " + shift.getDate().format(dateFormatter) + " " +
                        shift.getShiftType() + " (ID: " + shift.getId() + ") - " + managerName);
            }
        }
    }
