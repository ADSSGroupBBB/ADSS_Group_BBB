...
import dto.*;
import DataAccessDE.EmployeeInterface.*;
import DataAccessDE.EmployeeDAO.*;
...
private boolean hasAvailableShiftManagerForShift(LocalDate date, String shiftType, String branchAddress) {
    try {
        ...
        for (EmployeeDTO employee : branchEmployees) {
            ...
            if (employee.getRole() != Employee.UserRole.SHIFT_MANAGER && employee.getRole() != Employee.UserRole.HR_MANAGER) {
                continue;
            }
    ...
    private boolean assignShiftManagerAutomatically(ShiftDTO shift) {
        try {
            ...
            if (admin != null && admin.getRole() == Employee.UserRole.HR_MANAGER && !managerPositions.isEmpty()) {
